...
- **ndpa_writer.py**: Defines the `NDPAWriter` class for creating NDPA XML annotation files. Supports creating an empty NDPA document and incrementally adding rectangular bounding-box annotations in nanometer coordinates. Each annotation is written as an `ndpviewstate` entry with `title` (class label) and rectangle corner points under `annotation/pointlist`.
    - Example: `NDPAWriter(output_path)` creates an empty NDPA file, and `add_bounding_box(...)` appends one annotation.
...
writer = NDPAWriter("predictions.ndpi.ndpa")
