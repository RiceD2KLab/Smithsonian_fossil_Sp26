...
def _draw_box(img: np.ndarray, box: list, label: str, color: tuple) -> None:
...
def predict(args: argparse.Namespace) -> None:
    ...
    for img_path in img_paths:
        ...
        for i, (box, conf) in enumerate(zip(pred_boxes, pred_conf)):
            _draw_box(img_pred, box.tolist(), f"{float(conf):.2f}", _COL_PRED)
        ...
        if gt_dir is not None and gt_anns:
            img_gt = img_bgr.copy()
            for box in gt_boxes:
                _draw_box(img_gt, box.tolist(), "GT", _COL_GT)
        ...
        if fp_dir is not None and fn_dir is not None:
            ...
            if fp_mask.any():
                ...
                for pi in np.where(fp_mask)[0]:
                    _draw_box(img_fp, pred_boxes_s[pi].tolist(),
            ...
            if fn_mask.size and fn_mask.any():
                ...
                for gi in np.where(fn_mask)[0]:
                    _draw_box(img_fn, gt_boxes[gi].tolist(), "FN", _COL_FN)
